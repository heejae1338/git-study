...
```mermaid
깃헙에 브랜치 생성하고 싶으면 git branch 브랜치명
git switch 브랜치명
git add .
gut commit -m '커밋내용' 하고
git push 원격저장소주소 로컬브랜치명 입력하면 브랜치 저장됨
병합 요청은 pull request 요청하면 됨
```
...
- main 브랜치와 기능추가용 feature 브랜치만 운영
```

```mermaid
git stash를 사용해서 코드를 보관할 수 있음
최근 commit 상태로 파일들이 되돌아가고 변경된 코드들이 보관됨
staging 상관없이 추적중인 파일은 다 이동
git stash save "보관함제목" 처럼 메모 입력 가능
git stash list를  통해 코드 목록을 출력함
git stash pop 하면 보관했던 코드를 다시 불러옴
보관해놓은 코드가 여러개 있으면 가장 최근에 보관한 코드부터 불러옴
git stash drop 삭제할id 는 특정 stash 삭제
git stash clear 는 모든 stash 삭제 명령어
일부 코드만 git stash 하고싶으면 git stash -p 사용
```
